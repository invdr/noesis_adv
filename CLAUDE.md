...
Предстоит достроить под наружную рекламу (см. TASKS.md): каталог **конструкций**
(гео/формат/сторона/подсветка/охват/цена за период) на месте модели `Project`
(«ЖК»); **бронирование** конструкции по периодам (новая сущность) и сетка
занятости; **карта** города.

> Каталог, тексты сайта и демо-сид пока про недвижимость — плейсхолдер до
> редизайна домена. Модели `Project`/`Developer` временно сохранены как основа
> будущего каталога конструкций.
