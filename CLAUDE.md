...
Собрано на основе [tower-site](https://github.com/invdr/tower-site) (CRM +
лендинг застройщика). Переиспользовано домен-независимое ядро CRM; удалены
...
API (`/api/constructions`). Предстоит (см. TASKS.md): **бронирование** конструкции
по периодам (новая сущность) и сетка занятости; **карта** города; редизайн сайта.

> Тексты сайта и демо-сид пока про недвижимость — плейсхолдер до редизайна
> домена (Этап 5). Публичный лендинг временно работает через
> legacy-совместимость (`packages/contracts/src/legacy-site.ts` +
> `/api/public/projects` отдаёт старую ЖК-форму карточки из конструкций);
> удаляется при редизайне. `Developer` пока переиспользуется как «владелец сети».
...
— `--primary`). Публичный сайт — унаследованная от застройщика вёрстка; редизайн
под наружную рекламу запланирован (TASKS.md), делаем по явной задаче.
