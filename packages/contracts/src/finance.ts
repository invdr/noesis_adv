...
/** Баланс участника за период: начислено (по распределениям) − выплачено. */
export const financeParticipantBalanceSchema = z.object({
  participantId: z.string(),
  participantName: z.string(),
  kind: financeParticipantKindSchema,
  /** Начислено по закрытым распределениям периода. */
  allocated: z.number().int(),
  /** Выплачено за период. */
  paidOut: z.number().int(),
  /** Остаток к выплате (может быть отрицательным при переплате). */
