...
export const financeDistributionSchema = z.object({
  ...
  /**
   * Для закрытого месяца: живой итог поступлений/расходов разошёлся с
   * замороженным снимком (данные месяца поменялись после закрытия) — снимок
   * устарел, месяц нужно переоткрыть и закрыть заново. Для открытого — всегда false.
   */
  stale: z.boolean(),
