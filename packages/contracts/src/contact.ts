 ...
 *  - `client` — покупатель, к нему привязываются заявки (`Lead.contactId`);
 *    создаётся вручную или при приёме заявки, дедуп по телефону;
 *  - `realtor` / `agency` — партнёр, который ПРИВОДИТ покупателей
 *    (`Lead.referrerId`); заводится вручную в CRM, участвует в аналитике риелторов.
 * Риелтор может ссылаться на своё агентство (`agencyId`).
 */

/** Тип контакта. */
export const contactKindSchema = z.enum(["client", "realtor", "agency"]);
...
export const contactSchema = z.object({
  ...
  /** Агентство риелтора (kind=realtor → id контакта kind=agency); иначе null. */
  agencyId: z.string().nullable(),
  /** Имя агентства для показа (если `agencyId` задан). */
