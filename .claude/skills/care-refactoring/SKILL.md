...
> Claude Code note: where this skill says "subagent", "delegate to a subagent", or "challenge agent", use the **Agent (Task) tool** — each `Agent` call runs with a fresh, isolated context. If the Agent tool is unavailable, perform the same step yourself as a separate critical pass, as already described below.
