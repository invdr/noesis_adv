...
const envSchema = z
  .object({
    ...
    /**
     * Секрет для внутренней ручки рассылки напоминаний о сроке брони
     * (`/api/internal/bookings/reminders/notify`). Cron на хосте шлёт его в
     * заголовке `X-Reminder-Token`. Если не задан — ручка выключена (404),
     * Telegram-дайджест по броням не рассылается (веб-«Мой день» работает всегда).
     */
